...
pub fn main() !void {
    ...
    var port_p: ?serial.PortInformation = null;
    var i: u64 = 0;
    while (i <= 5) : (i += 1) {
        ...
        if (port_p == null) {
            if (i > 1) {
                std.debug.print(
                    "{d}/5.  Could not find: 0x{x} 0x{x} (retrying soon)\n",
                    .{ i, config.vid, config.pid },
                );
            }
    ...
    if (port_p == null) {
        std.debug.print("Failed Device Not Found: 0x{x}, 0x{x}\n", .{ config.vid, config.pid });
        return;
    }
...
const magic_cmd = [_]u8{config.magic_boot_char};
